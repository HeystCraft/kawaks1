...
bot.on("ready", async () => {
  var interval = setInterval (function () {
    let bstatut = ["Bienvenue", "Bonne journée", "Développer par Kawak's", "Abonnez vous a Kawak's LaunchPad"]
...
bot.on('guildMemberAdd', member => {
  member.createDM().then(channel => {
    return channel.send('Bienvenue sur le serveur de Kawak_s ' + member.displayName )
  }).catch(console.error)
...
bot.on('messageReactionAdd', (reaction, user) => {
  if(reaction.emoji.name === "✅") {
      if(reaction.message.channel.id === "439669478851346437") {
       //Remplacer channel.id === "ID de votre salon"
          user.send("**Vous avez accepté le réglement !**")
          const giveRole = reaction.message.guild.roles.find(r => r.id === "652073917782949906");
